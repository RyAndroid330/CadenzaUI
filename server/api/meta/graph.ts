...
export default defineEventHandler(async (event) => {
  ...
  const results = await Promise.allSettled([
    ...
    delegateQuery<Record<string, unknown>>(address, port, 'Query service', { filter: { is_meta: true }, sort: { name: 'asc' }, limit: 200 }),
    delegateQuery<Record<string, unknown>>(address, port, 'Query task', { filter: { is_meta: true }, sort: { name: 'asc' }, limit: 500 }),
    delegateQuery<Record<string, unknown>>(address, port, 'Query signal_registry', { filter: { is_meta: true }, sort: { name: 'asc' }, limit: 200 }),
